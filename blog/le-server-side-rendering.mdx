...
On peut faire un diagramme pour se représenter le fonctionnement :
...
- Si vous n'avez pas d'accès à une base de données
- Si l'application est juste un front connecté à une API déjà existante
...
| [QwikCity](https://qwik.builder.io/qwikcity/overview/)  | Qwik          | Node                   | Mieux, résumabilité\* | Oui                    | 📈                   |
...
- _CF Worker_ (CloudFlare) c'est un environnement spécial qui n'est pas Node et qui est fait pour tourner sur des CDN.
...
- SvelteKit c'est super cool, j'aime pas trop Svelte mais l'approche est super propre.
- SolidStart est loin d'être prêt encore, mais il promet beaucoup
- QwikCity utilise le système de résumabilité pour pallier aux problèmes d'hydratation, mais c'est un autre sujet tout à fait à part encore
- J'ai ajouté Angular Universal, mais sans blague, c'est pas fait pour faire du SSR comme je viens de le présenter, si vous utilisez Angular c'est du CSR qu'il vous faut quoi qu'il arrive.
