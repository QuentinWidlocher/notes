...
On va donc gérer des "projets", c'est très conceptuel mais au moins on va pouvoir jouer avec différentes complexitées.  
...
Il n'existe rien en HTML qui permette ça nativement et aucune convention globale, c'est donc à nous de definir un moyen d'interpréter des listes avec les moyens à notre disposition.
...
Ici on garde dans un état le nombre de champs, on boucle sur une liste vide juste pour créer le bon nombre de champs et on donne la possibilité d'incrémenter et de décrementer le nombre de champs.
...
Mais je m'emporte, passons à l'implémentation de ces champs infiniments imbriqués :
...
## `qs` le parser de querystring

Bien entendu, à chaque fois qu'il faudrait créer un sous-projet pour répondre à un besoin, on peut être sûr que des gens l'ont déjà probablement fait et c'est le cas avec [`qs`](https://github.com/ljharb/qs) qui a pour but de transformer une querystring :
...
Super ! Mais ça ne gère pas les valeurs numériques, booléenes et de date ça.  
...
Ensuite, pour fournir une querystring à `qs`, il va nous falloir transformer notre formData. Heureusement, il existe des méthode natives
...
Maintenant qu'on a cette querystring, on va pouvoir la parse à l'aide de `qs`
...
Tailwind y est déjà intégré ainsi que Prisma et SuperJSON, si vous avez des idées, n'hesitez pas à m'en parler sur la page github 😉
