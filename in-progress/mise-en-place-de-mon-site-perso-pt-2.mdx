...
Le découpage par features permet de très facilement regrouper des fonctionnalités sans pour autant "surdécouper". Ici, pas besoin d'avoir un dossier pour ma page "repos" et un autre pour ma page "stars" alors que ces deux pages partagent quelque chose comme 80% de leur code.
...
La configuration `@components/*` devait me permettre d'accéder facilement à n'importe quel composant, notamment depuis mes articles. Pour l'instant je n'ai pas eu besoin d'importer de composant autres que les bloc de codes à onglet que vous avec déjà pu voir.
...
Tout d'abord, le plus intéressant, le service worker lui même, appellé `sw.js` et servi comme un asset statique dans le dossier `./public`.  
  ...
  if (
    ...
    event.respondWith(
      ...
      caches.open(version).then(async (cache) => {
        // Si on trouve cette données dans notre cache, on la retourne simplement
...
Dans cette évenement, j'en profite pour supprimer du cache toutes les données qui ont probablement changé depuis ma dernière mise en production.

Pour purger le cache, j'utilise un système finalement très peu versatile mais extrèmement simple et efficace : un numéro de version.
...
Le seul bémol c'est qu'il faut se rappeler de l'incrémenter, mais cet désavantage est rapidement oublié car j'ai pris l'habitude d'incrémenter aussi la version dans le `package.json`.

#### Lancement du service worker

Le problème avec les service workers c'est qu'en mode développement c'est très désagréable d'avoir ce cache car les données changent forcément souvent et qu'il est hors de question d'incrémenter le numéro de version à chacun de ces changement.

Pour désactiver le lancement de mon service worker j'ai décidé d'ajouter un "lanceur" que j'ajoute au html de mes réponses ou pas, celui le mode (dev ou prod).

D'abord voici le lanceur :

```javascript
// On regarde sur le navigateur supporte les services workers quand même
if ("serviceWorker" in navigator) {
  // On attend le chargement de la page...
  window.addEventListener("load", () => {
    // ...et on enregistre le service worker
...
Encore une fois, c'est peut-être un peu smpliste, mais ça fonctionne très bien et je n'ai pas besoin de plus que ça.

#### Le manifest

Enfin, pour obtenir une belle application lors de l'installation de la PWA il faut fournir un fichier `manifest.json` qui contient des informations nécessaire comme le titre, la couleur, les icones etc. :
...
Qui sait, peut-être qu'un jour j'ajouterais des notifications natives pour tenir informés les gesn qui ont installé l'application (rassurez vous c'est optionnel)
