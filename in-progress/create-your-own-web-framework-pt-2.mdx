...
subtitle: The wheel we won't reinvent
...
Remember, in the last part, I made a list of all the things we won't have to create for our projet. In this list we can focus on the two main parts: File serving, custom code execution and templating
...
There are some problems here we'll have to fix later though, such as nested files (we only serve the files at the root of the director) or the name of the route (we need to go to `/page.html` to see a file named like that, instead of the usual `/page`)
...
If the files where `.js` files, we could simple `import` or `require()` them, then execute the code.  
...
To separate the code from the html template, I chose to put the server-side code inside a `<script/>` tag with a `server` attribute, which is html compliant (works with IDE and every other tools), but is specific enough so we won't interpret code that's not meant for the server.
...
function parseHtml(template: string): [html: string, serverScript: string] {
	...
	// Here [server] means "whith the server attribute"
...
Now that we have our serve code as a string, how can we run the code, so that we can print our example to the console ?
...
Both can run code based on a string, but the function constructor is scoped better, and can also take arguments while returning a result.

All in all, we should use the function constructor every time over `eval()`.
...
What if we _need_ values from the outside. Maybe we'd like to have access to the Express `req` parameter, which include a lot of useful informations like the URL parameters (things like `?query=something&page=1`), a route param (we'll look at that just after) or even the IP of the device requesting the page.
...
There exist a _lot_ of templating languages, such as [Handlebars](https://handlebarsjs.com/), [EJS](https://ejs.co/), [pug](https://pugjs.org/api/getting-started.html) etc.  
...
String templates helps reduce the complexity of such concatenation :
...
Here we used `.join('\n')` because `.map()` returns an array of strings and we want to display them, separated by a return.  
...
// Not how we don't use ` here, it's a regular string
...
<html lang="en">
	...
	<body>
		...
		display variable like this (this comment is also a real comment, but we NEED
		to return at least an empty string after the comment) */ ''}
		<h1>${title}</h1>
