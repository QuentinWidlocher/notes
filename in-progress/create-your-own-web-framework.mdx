...
Of course the answer to that is "mainly with PHP".  
I had to give a course on PHP a few days ago so I got back to it after several years of Javascript and it striked me how
...
That's when I thought, "wouldn't it be cool to use this old school approach with more modern tool".  
By "with more modern tool", undertand "with Javascript" of course. 😁

## What I want to do

I want to create a tool that's easy to learn and use, and makes a big use of the fundementals of Javascript and the web.
I won't create a web server so I'll use Express, but I'll try to leverage Express capatibilities the best I can, same goes for the templating system, I don't want to integrate JSX as I feel the \``template string ${interpolation}`\` is powerful enough.

This tool should allow beginners to build website easily, while having a good understanding of what's going on behind the scenes so I want to keep my source code as clear as possible.  
I know a lot of tools out there feels too magical for me and I really want to make the users feel like nothing can go wrong because they know all the tricks.

## What I don't want to do

I don't care if this tool will lack functionnality, or if it's too slow or whatever, I don't want to compete with Remix _at all_ (it's not like I could anyway 😂)
...
This tool is about learning first, but if it allows people to create cool websites too I'd be happy !

## How I want it to work

First, I made a list of all the functionality I think were needed in a web framework and split them into two groups :
...
It's kind of barebone but it's a great basis for the most advanced features coming next.

### The bonus

- Allowing layouts pages, or chunks of reusable templates (because writing the doctype each time is cumbersome)
...
- Refining the developper experience (errors display, watch mode etc.)
...
- Powerful and nested string interpolation with template litterals
- Asynchronous code easily with `async`/`await`
- An easy way to pass props around by `...spread`ing them to function arguments
- Librairies to fetch data and interract with files in the file system

Whew, that's a lot of things we _won't_ have to do right ? What's left then ?

- Serving html and assets based on their route
- Parsing the server code and getting back values to...
